...
public interface ClientCrudRepostirory extends CrudRepository<Client, Integer> {
    ...
    public void updMoney(@Param("id") long id, @Param("money") BigDecimal money);

    @Modifying
    @Query("update Client c set c.name = :name, c.surname = :surname, c.age = :age, c.sex = :sex where c.id = :id")
    public void updClientGeneralInfo(@Param("id") long id, @Param("name") String name, @Param("surname") String surname, @Param("age") int age, @Param("sex") String sex);

    @Modifying
    @Query("update Client c set c.email = :email, c.password = :password where c.id = :id")
    public void updClientLogInInfo(@Param("id") long id, @Param("email") String email, @Param("password") String password);
